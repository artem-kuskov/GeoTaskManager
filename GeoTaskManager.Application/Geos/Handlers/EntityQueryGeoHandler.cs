...
{
    public class EntityQueryGeoHandler
        : IRequestHandler<EntityQuery<_EntityType>,
            EntityResponse<_EntityType>>
    {
        private IMediator Mediator { get; }
        private ILogger<EntityQueryGeoHandler> Logger { get; }

        public EntityQueryGeoHandler(IMediator mediator,
            ILogger<EntityQueryGeoHandler> logger)
        {
            Mediator = mediator;
            Logger = logger;
        }
            ...
            CancellationToken cancellationToken)
        {
            Logger.LogInformation(AppLogEvent.HandleRequest,
                "Handle Get Geo {Request}", request.ToDictionary());

            if (request is null || string.IsNullOrWhiteSpace(request.Id))
            {
                Logger.LogWarning(AppLogEvent.HandleArgumentError,
                    "Handle Get Geo request with empty request");
